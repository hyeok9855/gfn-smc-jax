...
def kernel_func(
    x: Array, y: Array, signal_variance: Array, num_grid_per_dim: int, raw_length_scale: Array
) -> Array:
    ...
    normalized_distance = jnp.linalg.norm(x - y, 2) / (num_grid_per_dim * raw_length_scale)
...
def get_latents_from_white(white: Array, const_mean: Array, cholesky_gram: Array) -> Array:
...
def get_white_from_latents(latents: Array, const_mean: Array, cholesky_gram: Array) -> Array:
...
class LogGaussianCoxPines(Target):
    ...
    def __init__(
        ...
        def short_kernel_func(x, y):
            return kernel_func(x, y, self._signal_variance, self._num_grid_per_dim, self._beta)

        self._gram_matrix = gram(short_kernel_func, self._bin_vals)
        self._cholesky_gram = jnp.linalg.cholesky(self._gram_matrix)
        self._white_gaussian_log_normalizer = -0.5 * self._num_latents * jnp.log(2.0 * jnp.pi)
    ...
    def whitened_posterior_log_density(self, white: Array) -> Array:
        ...
        latent_function = get_latents_from_white(white, self._mu_zero, self._cholesky_gram)
    ...
    def unwhitened_posterior_log_density(self, latents: Array) -> Array:
        ...
        prior_log_density = -0.5 * jnp.sum(white * white) + self._unwhitened_gaussian_log_normalizer
        log_likelihood = poisson_process_log_likelihood(
