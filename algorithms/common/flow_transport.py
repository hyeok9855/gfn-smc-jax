...
class GeometricAnnealingSchedule(object):
    ...
    def __call__(self, step: int, samples: Samples):
        ...
        interpolated_densities = (1.0 - beta) * log_densities_initial + beta * log_densities_final
        return interpolated_densities


def get_delta_no_flow(samples: Samples, log_density: LogDensityByStep, step: int) -> Array:
...
def get_delta_path_grad(
    ...
    log_density_q = variational_density(jax.lax.stop_gradient(flow_params), transformed_samples)
...
def get_log_normalizer_increment_no_flow_verse(deltas: Array, log_weights: Array) -> Array:
    assert_equal_shape([deltas, log_weights])
    normalized_log_weights = jax.nn.log_softmax(log_weights)
    total_terms = normalized_log_weights + deltas
    assert_equal_shape([normalized_log_weights, log_weights, total_terms])
...
def get_log_normalizer_increment_no_flow_craft(deltas: Array, log_weights: Array) -> Array:
    assert_equal_shape([deltas, log_weights])
    normalized_log_weights = jax.nn.log_softmax(log_weights)
    total_terms = normalized_log_weights - deltas
    assert_equal_shape([normalized_log_weights, log_weights, total_terms])
