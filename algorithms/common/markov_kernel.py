...
def tree_add(tree_a, tree_b):
    ...
    return jax.tree_utils.tree_map(lambda a, b: a + b, tree_a, tree_b)


def tree_scalar_mul(tree, scalar):
    res = jax.tree_utils.tree_map(lambda x: x * scalar, tree)
...
def random_walk_metropolis(
    ...
    def rwm_step(previous_samples: Array, curr_key: RandomKey):
        ...
        samples_next = jnp.where(is_accepted[:, None], proposed_samples, previous_samples)
...
def momentum_step(
    samples_in: Array, momentum_in: Array, step_coefficient: Array, epsilon: Array, grad_log_density
) -> Array:
    ...
    momentum_out = tree_add(momentum_in, tree_scalar_mul(gradient_val, step_coefficient * epsilon))
...
def hmc_step(
    ...
    def scan_step(passed_state, unused_input):
        ...
        new_pos, new_mom = leapfrog_step(
            pos, mom, step_coefficient=1.0, epsilon=epsilon, grad_log_density=grad_log_density
        )
    ...
    new_state, _ = jax.lax.scan(scan_step, state_in, [None] * scan_length, length=scan_length)
    ...
    def get_combined_log_densities(pos, mom):
        ...
        mom_log_densities = jax.tree_util.tree_reduce(jnp.add, per_leaf_mom_log_densities)
...
def hmc(
    ...
    samples_final, acceptance_rates = jax.lax.scan(short_hmc_step, samples_in, step_keys)
