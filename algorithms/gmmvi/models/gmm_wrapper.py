...
def setup_gmm_wrapper(
    gmm: GMM, INITIAL_STEPSIZE, INITIAL_REGULARIZER, MAX_REWARD_HISTORY_LENGTH, INITIAL_LAST_ETA=-1
):
    ...
    def add_component(
        ...
        return GMMWrapperState(
            ...
            l2_regularizers=jnp.concatenate(
                (gmm_wrapper_state.l2_regularizers, jnp.ones(1) * INITIAL_REGULARIZER), axis=0
            ),
            last_log_etas=jnp.concatenate(
                (gmm_wrapper_state.last_log_etas, jnp.ones(1) * INITIAL_LAST_ETA), axis=0
            ),
            ...
            reward_history=jnp.concatenate(
                (
                    ...
                    jnp.ones((1, MAX_REWARD_HISTORY_LENGTH)) * jnp.finfo(jnp.float32).min,
                ),
                axis=0,
            ),
    ...
    def remove_component(gmm_wrapper_state: GMMWrapperState, idx: int):
        return GMMWrapperState(
            ...
            last_log_etas=jnp.concatenate(
                (gmm_wrapper_state.last_log_etas[:idx], gmm_wrapper_state.last_log_etas[idx + 1 :]),
                axis=0,
            ),
            num_received_updates=jnp.concatenate(
                (
            ...
            stepsizes=jnp.concatenate(
                (gmm_wrapper_state.stepsizes[:idx], gmm_wrapper_state.stepsizes[idx + 1 :]), axis=0
            ),
            reward_history=jnp.concatenate(
                (
        ...
        return GMMWrapperState(
            ...
            weight_history=jnp.concatenate(
                (
                    ...
                    jnp.expand_dims(jnp.exp(gmm_wrapper_state.gmm_state.log_weights), 1),
                ),
                axis=1,
            ),
    ...
    def update_rewards(gmm_wrapper_state: GMMWrapperState, rewards: chex.Array):
        return GMMWrapperState(
            ...
            stepsizes=gmm_wrapper_state.stepsizes,
            reward_history=jnp.concatenate(
                (gmm_wrapper_state.reward_history[:, 1:], jnp.expand_dims(rewards, 1)), axis=1
            ),
