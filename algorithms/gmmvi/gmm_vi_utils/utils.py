...
def reduce_weighted_logsumexp(
    ...
    if w is None:
        lswe = jax.nn.logsumexp(logx, axis=axis, keepdims=keep_dims)
    ...
    max_log_absw_x = jnp.where(
        jnp.isinf(max_log_absw_x), jnp.zeros([], max_log_absw_x.dtype), max_log_absw_x
    )
    wx_over_max_absw_x = jnp.sign(w) * jnp.exp(log_absw_x - max_log_absw_x)
    sum_wx_over_max_absw_x = jnp.sum(wx_over_max_absw_x, axis=axis, keepdims=keep_dims)
    if not keep_dims:
