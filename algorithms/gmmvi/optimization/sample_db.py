...
def setup_sampledb(
    ...
    def add_samples(
        ...
        if (
            ...
            and jnp.shape(new_samples)[0] + jnp.shape(sampledb_state.samples)[0] > MAX_SAMPLES
        ):
        ...
        if KEEP_SAMPLES:
            mapping = jnp.concatenate(
                (sampledb_state.mapping, new_mapping + jnp.shape(sampledb_state.chols)[0])
            )
    ...
    def _gaussian_log_pdf(mean, chol, inv_chol, x):
        if DIAGONAL_COVS:
            ...
            return constant_part - 0.5 * jnp.sum(
                jnp.square(
                    jnp.expand_dims(inv_chol, 1) * jnp.transpose(jnp.expand_dims(mean, 0) - x)
                ),
                axis=0,
            )
        else:
