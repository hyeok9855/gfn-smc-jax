...
def setup_quad_regression(DIM):
    ...
    def fit_quadratic(
            ...
            def _feature_fn(quad_reg_state, num_samples: int, x: chex.Array) -> chex.Array:
                ...
                for i in range(quad_reg_state.dim):
                    quad_features = jnp.concatenate(
                        (quad_features, jnp.expand_dims(x[:, i], axis=1) * x[:, i:]), axis=1
                    )
        ...
        qt[quad_reg_state.triu_idx_const[:, 0], quad_reg_state.triu_idx_const[:, 1]] = params[
            -(DIM + 1)
        ]
    ...
    return QuadRegression(init_quad_reg_state=init_quad_reg_state, fit_quadratic=fit_quadratic)
