    ...
    def update_stepsize(gmm_wrapper_state: GMMWrapperState) -> chex.Array:
        ...
        for i in range(jnp.shape(gmm_wrapper_state.stepsizes)[0]):
            new_stepsize = INITIAL_STEPSIZE / (
                1
                + jax.lax.pow(float(gmm_wrapper_state.num_received_updates[i]), ANNEALING_EXPONENT)
            )
...
def setup_improvement_based_stepsize_adaptation(
    MIN_STEPSIZE: float, MAX_STEPSIZE: float, STEPSIZE_INC_FACTOR: float, STEPSIZE_DEC_FACTOR: float
):
