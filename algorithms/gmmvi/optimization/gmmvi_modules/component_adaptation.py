...
def setup_vips_component_adaptation(
    ...
    KERNEL = jnp.exp(
        GAUSSIAN.log_prob(jnp.arange(start=-FILTER_DELAY, stop=FILTER_DELAY, dtype=jnp.float32))
    )
    ...
    def _sample_from_prior(num_samples: int, seed):
        return jnp.transpose(
            ...
            + jnp.expand_dims(PRIOR_CHOL_COV, -1) * jax.random.normal(seed, (DIM, num_samples))
        )

    def _select_samples_for_adding_heuristic(
        component_adaption_state: ComponentAdaptationState, sample_db_state: SampleDBState, seed
    ):
        ...
        if NUM_PRIOR_SAMPLES > 0:
            ...
            sample_db_state = sample_db.update_num_samples_written(
                sample_db_state, (sample_db_state.num_samples_written + NUM_PRIOR_SAMPLES)
            )
        component_adaption_state = ComponentAdaptationState(
    ...
    def _add_new_component(
        ...
        key, subkey = jax.random.split(key)
        component_adaption_state, sample_db_state, samples, target_lnpdfs, prior_samples = (
            _select_samples_for_adding_heuristic(component_adaption_state, sample_db_state, subkey)
        )
        if NUM_PRIOR_SAMPLES > 0:
        ...
        return (
            _add_at_best_location(
                component_adaption_state, gmm_wrapper_state, samples, target_lnpdfs, subkey
            ),
    ...
    def _delete_bad_components(
        components_adaption_state: ComponentAdaptationState, gmm_wrapper_state: GMMWrapperState
    ):
        ...
        current_smoothed_reward = jnp.mean(
            gmm_wrapper_state.reward_history[:, -jnp.size(KERNEL) :] * jnp.reshape(KERNEL, [1, -1]),
            axis=1,
        )
        old_smoothed_reward = jnp.mean(
            gmm_wrapper_state.reward_history[:, -jnp.size(KERNEL) - DEL_ITERS : -DEL_ITERS]
        ...
        max_actual_weights = jnp.max(
            gmm_wrapper_state.weight_history[:, -jnp.size(KERNEL) - DEL_ITERS : -1], axis=1
        )
        ...
        is_old_enough = (
            gmm_wrapper_state.reward_history[:, -DEL_ITERS] != -jnp.finfo(jnp.float32).max
        )
    ...
    def adapt_number_of_components(
        ...
        if iteration > 1 and iteration % ADD_ITERS == 0:
            if gmm_wrapper_state.gmm_state.num_components < MAX_COMPONENTS:
                gmm_wrapper_state, component_adaption_state, sample_db_state = _add_new_component(
                    component_adaption_state, sample_db_state, gmm_wrapper_state, seed
                )
