...
def setup_get_expected_log_ratios(
    ...
    def get_expected_log_ratios(
        gmm_wrapper_state: GMMWrapperState, samples, background_mixture_densities, target_lnpdfs
    ):
...
def setup_update_weights_fn(
    get_expected_log_ratios_fn: Callable, update_weights_from_expected_log_ratios_fn: Callable
):
...
def setup_direct_weight_updater(
    ...
    def _update_weights_from_expected_log_ratios(
        gmm_wrapper_state: GMMWrapperState, expected_log_ratios: chex.Array, stepsize: jnp.float32
    ):
...
def setup_trust_region_based_weight_updater(
    gmm_wrapper: GMMWrapper, TEMPERATURE: float, USE_SELF_NORMALIZED_IMPORTANCE_WEIGHTS: bool
):
    ...
    def _kl(
        gmm_wrapper_state: GMMWrapperState, eta: jnp.float32, component_rewards: chex.Array
    ) -> [jnp.float32, chex.Array]:
        unnormalized_weights = (eta + 1) / (
            TEMPERATURE + eta
        ) * gmm_wrapper_state.gmm_state.log_weights + 1.0 / (TEMPERATURE + eta) * component_rewards
        ...
        kl = jnp.sum(
            jnp.exp(new_log_weights) * (new_log_weights - gmm_wrapper_state.gmm_state.log_weights)
        )
    ...
    def _update_weights_from_expected_log_ratios(
        ...
        def _bracketing_search(gmm_wrapper_state, expected_log_ratios, KL_BOUND):
            ...
            def body_fn(carry):
                it, lower_bound, upper_bound, log_eta, _, upper_bound_satisfies_constraint, _ = (
                    carry
                )
                eta = jnp.exp(log_eta)
                ...
                def false_fn():
                    new_lower_bound, new_upper_bound, new_upper_bound_satisfies_constrain = (
                        jax.lax.cond(
                            KL_BOUND > kl,
            ...
            return jax.lax.cond(
                lower_bound == upper_bound, lambda: (kl, eta, new_log_weights), get_return_values_fn
            )
