...
def setup_direct_ng_based_component_updater(
    ...
    def apply_ng_update(
        ...
        for i in range(gmm_wrapper_state.gmm_state.num_components):
            ...
            delta_lin = expected_hessians_neg[i] @ jnp.expand_dims(old_mean, 1) - jnp.expand_dims(
                expected_gradients_neg[i], 1
            )
...
def setup_kl_constrained_ng_based_component_updater(
    ...
    def _kl(
        ...
        eta = jax.lax.cond(eta_in_logspace, lambda eta: jnp.exp(eta), lambda eta: eta, eta)
        ...
        if DIAGONAL_COVS:
            ...
            kl = 0.5 * (
                jnp.maximum(
                    0.0,
                    jnp.sum(jnp.log(new_precision / old_precision) + old_precision / new_precision)
                    - DIM,
                )
            ...
            def false_fn():
                new_mean = jnp.reshape(
                    jax.scipy.linalg.cho_solve((chol_precision, True), jnp.expand_dims(new_lin, 1)),
                    [-1],
                )
    ...
    def _bracketing_search(
        ...
        def body_fn(carry):
            ...
            def false_fn():
                new_upper_bound, new_lower_bound, new_upper_bound_satisfies_constraint = (
                    jax.lax.cond(
                        KL_BOUND > kl,
        ...
        _, lower_bound, upper_bound, eta, kl, upper_bound_satisfies_constraint = jax.lax.while_loop(
            cond_fn,
            body_fn,
            init_val=(0, lower_bound, upper_bound, 0.5 * (upper_bound + lower_bound), -1000, False),
        )
    ...
    def apply_ng_update(
        ...
        def _apply_gn_update_per_comp(
            ...
            if DIAGONAL_COVS:
                ...
                reward_lin = (
                    jnp.squeeze(reward_quad @ jnp.expand_dims(old_mean, 1)) - expected_gradients_neg
                )
            ...
            lower_bound_const, upper_bound_const = jax.lax.cond(
                ...
                lambda last_eta: (jnp.maximum(0.0, jnp.log(last_eta) - 3), jnp.log(last_eta) + 3),
                last_eta,
            )
            ...
            def true_lower_equals_upper():
                ...
                return jax.lax.cond(
                    (new_kl < jnp.finfo(jnp.float32).max) & (~jnp.any(jnp.isnan(new_chol))),
