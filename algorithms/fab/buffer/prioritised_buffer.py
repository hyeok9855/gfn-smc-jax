...
def build_prioritised_buffer(
    dim: int, max_length: int, min_length_to_sample: int, sample_with_replacement: bool = False
) -> PrioritisedBuffer:
    """
    ...
    def init(x: chex.Array, log_w: chex.Array, log_q_old: chex.Array) -> PrioritisedBufferState:
        ...
        buffer_state = PrioritisedBufferState(
            data=data, is_full=is_full, can_sample=can_sample, current_index=current_index
        )
        buffer_state = add(x, log_w, log_q_old, buffer_state)
        return buffer_state

    def add(
        x: chex.Array, log_w: chex.Array, log_q: chex.Array, buffer_state: PrioritisedBufferState
    ) -> PrioritisedBufferState:
        """Update the buffer's state with a new batch of data."""
        chex.assert_rank(x, 2)
        ...
        valid_samples = (
            jnp.isfinite(log_w) & jnp.all(jnp.isfinite(x), axis=-1) & jnp.isfinite(log_q)
        )
        indices = (jnp.arange(batch_size) + buffer_state.current_index) % max_length

        # Remove invalid samples.
        x, log_w, log_q = jax.tree_utils.tree_map(
        ...
        can_sample = jax.lax.select(
            buffer_state.is_full, buffer_state.can_sample, new_index >= min_length_to_sample
        )
        current_index = new_index % max_length

        data = Data(x=x, log_w=log_w, log_q_old=log_q)
        state = PrioritisedBufferState(
            data=data, current_index=current_index, is_full=is_full, can_sample=can_sample
        )
        return state
    ...
    def sample_n_batches(
        key: chex.PRNGKey, buffer_state: PrioritisedBufferState, batch_size: int, n_batches: int
    ) -> Iterable[Tuple[chex.Array, chex.Array, chex.Array]]:
        """Returns dataset with n-batches on the leading axis."""
        x, log_q_old, indices = sample(key, buffer_state, batch_size * n_batches)
        dataset = jax.tree_utils.tree_map(
            lambda x: x.reshape((n_batches, batch_size, *x.shape[1:])), (x, log_q_old, indices)
        )
        return dataset
    ...
    def adjust(
        ...
        # remove invalid samples
        log_w, log_q = jax.tree_utils.tree_map(
